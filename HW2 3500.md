...
### 3.4) Best subset $O(n)$
When we run `best_subset_middle()` we know that the optimal solution will always be contained in the resulting interval (that passes through $\frac{n}{2}$) which we will call $I$. Since the optimal interval is by definition the interval that has the largest sum, BSM will include the elements that contribute most to the sum. 

There are optimizations that could be made to make the 
```python
